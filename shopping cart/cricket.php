...
<!-- View Cart Box Start -->
<?php
//Checks if a cart session has been started and any products in the cart
if(isset($_SESSION["cart_products"]) && count($_SESSION["cart_products"])>0)
{
	...
	foreach ($_SESSION["cart_products"] as $cart_itm)
	{
		...
		$bg_color = ($b++%2==1) ? 'odd' : 'even'; //shows light grey for even and dark grey for odd
...
<!-- Products List Start -->
<?php
$results = $mysqli->query("SELECT id, product_code, product_name, product_price FROM products WHERE category='Cricket' ORDER BY id ASC ");
if($results){ 
$products_item = '<ul class="products">';
//fetch results set as object and output HTML
while($obj = $results->fetch_object())
{
	//when EOT is used you dont need echo and the font size with be higher
$products_item .= <<<EOT
	<li class="product">
